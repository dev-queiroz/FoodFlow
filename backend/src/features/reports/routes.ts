...
router.get(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            const report = await reportService.generateSessionReport(req.params.restaurantId, (req as any).userId, filter);
            const format = req.query.format as string | undefined;
            if (format === 'pdf') {
                const pdfBuffer = await reportService.generateSessionReportPDF(report);
                res.setHeader('Content-Type', 'application/pdf');
                res.setHeader('Content-Disposition', 'attachment; filename=session_report.pdf');
                return res.send(pdfBuffer);
            } else if (format === 'csv') {
                const csvContent = await reportService.generateSessionReportCSV(report);
                res.setHeader('Content-Type', 'text/csv');
                res.setHeader('Content-Disposition', 'attachment; filename=session_report.csv');
                return res.send(csvContent);
            }
            res.json(report);
        } catch (err: any) {
